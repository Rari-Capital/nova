...
export class Watcher {
  ...
  public async getL1RelayTransaction(
    l2ToL1MsgHash: string,
    pollForPending: boolean = true
  ): Promise<ContractTransaction> {
    return this.getTransaction(this.l2, l2ToL1MsgHash, pollForPending);
  }

  public async getL2RelayTransaction(
    l1ToL2MsgHash: string,
    pollForPending: boolean = true
  ): Promise<ContractTransaction> {
    return this.getTransaction(this.l2, l1ToL2MsgHash, pollForPending);
  }
  ...
  public async getTransaction(
    ...
    if (matches.length > 0) {
      ...
      return layer.provider.getTransaction(matches[0].transactionHash);
    }
    if (!pollForPending) {
    ...
    return new Promise(async (resolve, reject) => {
      layer.provider.on(filter, async (log: any) => {
        if (log.data === msgHash) {
          try {
            const tx = await layer.provider.getTransaction(log.transactionHash);
